    ...
    <>
      <Section className="dashboard-section dashboard-section--hero">
        <WorkstationPageHeader
          ...
          actions={[
            { href: '/invest/etfs', label: 'Back to ETFs' },
          ...
          <div className="analytics-side-stack">
            <Card className="analytics-card">
              ...
              <div className="analytics-card__action-grid">
                ...
                />
                <SimulatedOrderForm assetId={asset.asset.assetId} symbol={asset.asset.symbol} assetClass="etf" side="buy" strategyLaneId="manual_multi_asset_lane" simulationSessionId={sessionContext?.sessionId ?? undefined} label={messages.dashboard.buySimulated} showQuantityInput quantityLabel={messages.simulation.quantity} disabled={tradingDisabled} disabledReason={disabledReason} currentPrice={asset.quote?.price ?? null} currentHeldQuantity={asset.position?.quantity ?? 0} sourceContext="etf-lane" uiText={{ quantityMode: messages.simulation.quantity, notionalMode: 'Notional', notionalAmount: 'Notional amount', quantityRequired: messages.simulation.validation.quantityRequired, minimumShare: messages.simulation.validation.minimumShare, minimumUnit: messages.simulation.validation.minimumUnit, minimumQuantityTemplate: messages.simulation.validation.minimumQuantity, wholeSharesOnly: messages.simulation.validation.wholeSharesOnly, quantityStepMismatchTemplate: messages.simulation.validation.quantityStepMismatch, minimumNotional: messages.simulation.validation.minimumNotional, noOpenPositionToSellTemplate: messages.simulation.validation.noOpenPositionToSell, closePosition: messages.simulation.chips.closePosition }} />
                <SimulatedOrderForm assetId={asset.asset.assetId} symbol={asset.asset.symbol} assetClass="etf" side="sell" strategyLaneId="manual_multi_asset_lane" simulationSessionId={sessionContext?.sessionId ?? undefined} label={messages.dashboard.sellSimulated} showQuantityInput quantityLabel={messages.simulation.quantity} disabled={tradingDisabled || !asset.position || asset.position.quantity <= 0} disabledReason={!asset.position || asset.position.quantity <= 0 ? messages.simulation.validation.noOpenPositionToSell.replace('{{symbol}}', asset.asset.symbol) : disabledReason} currentPrice={asset.quote?.price ?? null} currentHeldQuantity={asset.position?.quantity ?? 0} sourceContext="etf-lane" uiText={{ quantityMode: messages.simulation.quantity, notionalMode: 'Notional', notionalAmount: 'Notional amount', quantityRequired: messages.simulation.validation.quantityRequired, minimumShare: messages.simulation.validation.minimumShare, minimumUnit: messages.simulation.validation.minimumUnit, minimumQuantityTemplate: messages.simulation.validation.minimumQuantity, wholeSharesOnly: messages.simulation.validation.wholeSharesOnly, quantityStepMismatchTemplate: messages.simulation.validation.quantityStepMismatch, minimumNotional: messages.simulation.validation.minimumNotional, noOpenPositionToSellTemplate: messages.simulation.validation.noOpenPositionToSell, closePosition: messages.simulation.chips.closePosition }} />
              </div>
            </Card>
            <DetailSlotCard
              eyebrow="ETF profile"
              title="Benchmark and allocation context"
              description="ETF detail lanes keep benchmark context and simulation actions together."
              items={[
          ...
          <Card className="analytics-card">
            <div className="analytics-card__header">
              <div>
                ...
                <p>Signed-in users see live-marked holding context directly on the ETF detail page.</p>
              </div>
            </div>
            <div className="analytics-card__body">
          ...
          <Card className="analytics-card">
            <div className="analytics-card__header">
              <div>
                ...
                <h3>Continue benchmark workflow</h3>
                <p>Compare ETF context in the lane view, then place simulation orders with lane safety gates.</p>
              </div>
            </div>
            <div className="analytics-card__action-grid">
              <Link href="/invest/etfs" className="button button--secondary">
                Browse more ETFs
              </Link>
