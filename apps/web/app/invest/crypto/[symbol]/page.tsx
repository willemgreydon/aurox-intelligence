...
export async function generateMetadata({ params }: CryptoDetailPageProps): Promise<Metadata> {
  ...
  return {
    title: `${upperSymbol} Crypto Intelligence | Aurox Intelligence`,
    description: `Simulation-mode research and order planning for ${upperSymbol}. 24/7 quote context, signal scores, risk overlay, and paper-trading actions.`,
  };
}
...
export default async function CryptoDetailPage({ params, searchParams }: CryptoDetailPageProps) {
  ...
  return (
    <SymbolDetailView
      ...
      basePath={`/invest/crypto/${encodeURIComponent(asset.asset.symbol)}`}
      backHref="/invest/crypto"
