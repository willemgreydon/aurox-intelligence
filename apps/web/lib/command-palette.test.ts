...
const entries: CommandPaletteEntry[] = [
  { id: 'x', title: 'Dashboard', href: '/dashboard', group: 'Routes', keywords: ['executive', 'cockpit'] },
...
describe('command-palette search', () => {
  ...
  it('includes dashboard route in results', () => {
    const rows = searchCommandPalette(entries, 'dashboard');
    expect(rows.some((row) => row.href === '/dashboard')).toBe(true);
  });
});
