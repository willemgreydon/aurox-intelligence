...
type SessionRow = {
  ...
  maxCapitalUsd: number | string;
  microAllocationPercent: number | string;
  decisionSource: 'manual_ui' | 'ai_assisted' | 'automation';
...
type StartSimulationSessionInput = SessionSelector & {
  laneMode: SimulationLaneMode;
  assetScope: SimulationAssetScope;
...
function mapSessionRow(row: SessionRow): SimulationSession {
...
async function listSimulationSessionsByPreference(client: DatabaseClient, userId: string): Promise<SimulationSession[]> {
...
export async function listSimulationSessionsForUser(userId: string): Promise<SimulationSession[]> {
  const client = createDatabaseClient();
  assertDatabaseConfigured(client);
  return listSimulationSessionsByPreference(client, userId);
}

export async function getSimulationSessionByIdForUser(userId: string, sessionId: string): Promise<SimulationSession | null> {
  const client = createDatabaseClient();
  assertDatabaseConfigured(client);
...
export async function getPreferredSimulationSessionForUser(
  userId: string,
...
export async function startOrResumeSimulationSession(input: StartSimulationSessionInput): Promise<SimulationSession> {
  const client = createDatabaseClient();
  assertDatabaseConfigured(client);

  return client.transaction(async (transactionClient) => {
    const [activeRow] = await transactionClient.query<SessionRow>(
      `
        select
          id,
          user_id as "userId",
    ...
    if (activeRow) {
      const [updatedRow] = await transactionClient.query<SessionRow>(
        `
    ...
    const [createdRow] = await transactionClient.query<SessionRow>(
      `
...
export async function markSimulationSessionOpened(userId: string, sessionId: string): Promise<void> {
  const client = createDatabaseClient();
  assertDatabaseConfigured(client);
...
export async function updateSimulationObservationHealth(): Promise<number> {
  const client = createDatabaseClient();
  assertDatabaseConfigured(client);
