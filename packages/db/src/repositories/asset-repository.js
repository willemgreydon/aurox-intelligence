...
import { investmentUniverse, searchInvestmentUniverse } from './investment-universe-repository';
const marketAssetsTable = 'app.market_assets';
function isMissingMarketAssetsError(error) {
...
function buildMarketAssetId(symbol, assetClass) {
    return `${assetClass}-${symbol.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
}
export async function upsertMarketAssets(inputs) {
    if (inputs.length === 0) {
        return;
    }
...
export function isAssetSimulationTradable(asset) {
    if (!asset) {
        return false;
    }
...
function mapFallbackAsset(asset) {
    return {
        ...asset,
        isTradable: asset.assetClass === 'stock' ||
            asset.assetClass === 'etf' ||
            asset.assetClass === 'crypto',
    };
}
function mapMarketAsset(row) {
    return {
        assetId: row.assetId,
        symbol: row.symbol,
        name: row.name,
...
function listFallbackAssets(assetClass) {
    return investmentUniverse
        .filter((asset) => (assetClass ? asset.assetClass === assetClass : true))
        .map(mapFallbackAsset);
}
function mergeCatalogAssetsWithFallback(fallbackAssets, persistedAssets) {
...
export async function listCatalogAssets(assetClass) {
    const client = createDatabaseClient();
    if (!client.isConfigured) {
...
export async function listTradableCatalogAssets() {
    const assets = await listCatalogAssets();
    return assets.filter((asset) => asset.isTradable);
}
export async function listSimulationTradableAssets(assetScope = 'multi-asset') {
    const assets = await listCatalogAssets();
...
export async function searchStockAssets(query) {
    const normalized = query.trim().toLowerCase();
    const assets = await listStockAssets();
    if (!normalized) {
        return assets;
    }
    const matchedSymbols = new Set(searchInvestmentUniverse(normalized, { assetClass: 'stock' }).map((asset) => asset.symbol));
    return assets.filter((asset) => matchedSymbols.has(asset.symbol));
}
export async function getCatalogAssetBySymbol(symbol) {
