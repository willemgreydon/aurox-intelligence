const curatedInvestmentUniverse = [
    {
        assetId: 'stock-aapl',
        symbol: 'AAPL',
        name: 'Apple',
...
function makeFallbackAsset(symbol, assetClass) {
    return {
...
function buildDefaultMetadataTags(asset) {
    const tags = new Set([asset.assetClass, 'universe']);
...
function buildDefaultProviderSymbolMap(asset) {
    if (asset.assetClass === 'crypto') {
        return {
...
function buildDefaultBrokerIdentifierMap(asset) {
    if (asset.assetClass === 'crypto') {
...
function mergeExpandedUniverse(base) {
...
export const investmentUniverse = mergeExpandedUniverse(curatedInvestmentUniverse).map(normalizeInvestmentAsset);
export function toCanonicalAssetMetadata(asset) {
    return {
        id: asset.assetId,
        symbol: asset.symbol,
...
export function getCanonicalInvestmentMetadataMap() {
    return new Map(investmentUniverse.map((asset) => [asset.symbol, toCanonicalAssetMetadata(asset)]));
}
export function searchInvestmentUniverse(query, options) {
