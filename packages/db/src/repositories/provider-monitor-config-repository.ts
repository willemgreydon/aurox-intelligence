...
type Row = {
  ...
  providerKey: string;
  providerName: string;
  category: MonitoredProviderConfig['category'];
  enabled: boolean;
...
export function defaultMonitorConfigs(): MonitoredProviderConfig[] {
...
export async function listProviderMonitorConfigs(): Promise<MonitoredProviderConfig[]> {
  const client = createDatabaseClient();
  if (!client.isConfigured) return defaultMonitorConfigs();
  try {
    ...
    if (rows.length === 0) return defaultMonitorConfigs();
    ...
    if (isMissingTable(error)) return defaultMonitorConfigs();
    throw error;
  }
}

export async function saveProviderMonitorConfigs(configs: MonitoredProviderConfig[]): Promise<MonitoredProviderConfig[]> {
  const parsed = configs.map((config) => monitoredProviderConfigSchema.parse(config));
  const client = createDatabaseClient();
  if (!client.isConfigured) return parsed;
  try {
    ...
    if (isMissingTable(error)) return parsed;
    throw error;
  }
}
