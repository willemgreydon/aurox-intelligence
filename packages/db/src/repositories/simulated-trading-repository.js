...
import { simulationAccountSummarySchema, simulationExecutionModelSchema, simulationExecutionRecordSchema, simulationExecutionInputSchema, simulationOrderSchema, simulationPositionSchema, simulationSnapshotSchema, simulationTransactionSchema, simulationWorkspaceSchema, } from '@repo/api-contracts';
...
function normalizeExecutionModel(model) {
...
function applySlippage(requestedPrice, side, slippageBps) {
...
function extractExecutionRecordFromNotes(notes) {
    if (!notes) {
        return null;
    }
    let payload = notes;
...
async function ensureSimulationAccount(client, userId) {
...
export async function hasSimulationAccountForUser(userId) {
...
async function captureSnapshot(client, accountId, portfolioId) {
    const [accountRow] = await client.query(`
      ...
      limit 1
    `, [accountId]);
    const [positionAggregate] = await client.query(`
      select
...
export async function listSimulatedOrdersForUser(userId) {
    const client = createDatabaseClient();
    assertDatabaseConfigured(client);
...
export async function insertSimulationAgentDecision(input) {
    const client = createDatabaseClient();
    assertDatabaseConfigured(client);
...
export async function linkSimulationAgentDecisionToOrder(input) {
    const client = createDatabaseClient();
    assertDatabaseConfigured(client);
...
export async function getTodayAiSimulationOrderNotionalForUser(userId, strategyTag = DEFAULT_AI_SIMULATION_STRATEGY_TAG, laneId = null) {
    const client = createDatabaseClient();
    assertDatabaseConfigured(client);
    const account = await ensureSimulationAccount(client, userId);
    ...
    try {
        ...
        try {
            ...
            throw new Error(`Unable to determine AI simulation daily notional usage safely. ` +
                `Structured query failed: ${structuredError instanceof Error ? structuredError.message : String(structuredError)}. Fallback query failed: ${fallbackError instanceof Error ? fallbackError.message : String(fallbackError)}.`);
        }
    }
}
export async function listSimulationTransactionsForUser(userId) {
    const client = createDatabaseClient();
    assertDatabaseConfigured(client);
    const account = await ensureSimulationAccount(client, userId);
    const rows = await client.query(`
...
export async function listSimulationSnapshotsForUser(userId) {
    const client = createDatabaseClient();
    assertDatabaseConfigured(client);
    const account = await ensureSimulationAccount(client, userId);
    const rows = await client.query(`
...
export async function getSimulationWorkspace(userId, marketPrices = {}) {
    const client = createDatabaseClient();
    assertDatabaseConfigured(client);
    const account = await ensureSimulationAccount(client, userId);
    ...
    const mappedClosedPositions = closedPositions.map((row) => simulationPositionSchema.parse({
        id: row.id,
        assetId: row.assetId,
        symbol: row.symbol,
        assetClass: row.assetClass,
    ...
    const summary = simulationAccountSummarySchema.parse({
        accountId: account.accountId,
...
export async function getSimulationWorkspaceIfExists(userId, marketPrices = {}) {
...
export async function createSimulatedOrder(input) {
...
export async function executeSimulationOrder(input) {
    ...
    return client.transaction(async (transactionClient) => {
        ...
        const executionRecord = simulationExecutionRecordSchema.parse({
            ...
            requestedPrice,
            executionPrice,
            slippageAmount: parsed.side === 'buy'
        ...
        const [lockedAccount] = await transactionClient.query(`
        select
          cash_balance as "cashBalance",
        ...
        for update
      `, [account.accountId]);
        const [currentPosition] = await transactionClient.query(`
        select
          id,
          asset_id as "assetId",
          symbol,
      ...
      `, [
        ...
        const [createdOrder] = await transactionClient.query(`
        select
          id,
          asset_id as "assetId",
          symbol,
...
export async function resetSimulationAccount(userId) {
    const client = createDatabaseClient();
    assertDatabaseConfigured(client);
    return client.transaction(async (transactionClient) => {
...
export async function captureSimulationSnapshotsForAllAccounts() {
    const client = createDatabaseClient();
    assertDatabaseConfigured(client);
