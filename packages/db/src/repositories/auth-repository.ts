...
type AuthSessionJoinRow = SessionRow & {
  userRecordId: string;
  userEmail: string;
  ...
  userEmailVerifiedAt: string | Date | null;
  userCreatedAt: string | Date;
  userUpdatedAt: string | Date;
};

export type AuthUserRecord = AccountUser & {
  passwordHash: string | null;
  status: UserStatus;
...
function assertDatabaseConfigured(client: DatabaseClient) {
  if (!client.isConfigured) {
    throw new Error(
      'DATABASE_URL must point to a Postgres database to use the production auth/session repository.',
    );
  }
}
...
function mapJoinedUser(row: AuthSessionJoinRow): AuthUserRecord {
  return {
    id: row.userRecordId,
    email: row.userEmail,
...
function mapVerificationToken(row: VerificationTokenRow): VerificationTokenRecord {
  return {
    id: row.id,
    userId: row.userId,
    email: row.email,
...
async function findUserByIdWithClient(client: DatabaseClient, id: string) {
...
async function findUserByEmailWithClient(client: DatabaseClient, email: string) {
  const rows = await client.query<UserRow>(
    `
      select
        id,
...
async function findAuthenticatedSessionByTokenHashWithClient(
  client: DatabaseClient,
  tokenHash: string,
) {
  const rows = await client.query<AuthSessionJoinRow>(
    `
      select
        s.id,
        s.user_id as "userId",
  ...
  } satisfies AuthenticatedSessionRecord;
}
...
export async function findAuthenticatedSessionByTokenHash(tokenHash: string) {
  const client = createDatabaseClient();
  assertDatabaseConfigured(client);
...
export async function deleteAuthSessionById(sessionId: string) {
  return revokeSession(sessionId);
}

export async function revokeAllUserSessions(userId: string, exceptSessionId?: string | null) {
  const client = createDatabaseClient();
  assertDatabaseConfigured(client);
...
export async function countActiveAuthSessionsForUser(userId: string) {
  const client = createDatabaseClient();
  assertDatabaseConfigured(client);
...
export async function listRecentAuthSessionsForUser(
  userId: string,
...
export async function createVerificationToken(input: {
  ...
  } satisfies VerificationTokenRecord;
}
...
export async function consumeVerificationToken(input: {
  ...
  return client.transaction(async (transactionClient) => {
    const tokenHash = hashOpaqueToken(input.token);
    const rows = await transactionClient.query<VerificationTokenRow>(
      `
        select
          id,
          user_id as "userId",
...
export async function resetPasswordFromToken(token: string, passwordHash: string) {
  const client = createDatabaseClient();
  assertDatabaseConfigured(client);
...
async function consumeVerificationTokenWithinClient(
  client: DatabaseClient,
  token: string,
  type: VerificationTokenType,
) {
