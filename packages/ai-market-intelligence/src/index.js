...
function mapStance(changePercent, forecastBias) {
...
function buildConfidence(input) {
    ...
    const freshnessComponent = input.freshnessState === 'live' ? 0.22 : input.freshnessState === 'delayed' ? 0.14 : input.freshnessState === 'partial' ? 0.08 : 0.04;
...
function formatMove(changePercent) {
    if (changePercent === null) {
        return 'move unavailable';
    }
    return `${changePercent > 0 ? '+' : ''}${changePercent.toFixed(2)}%`;
}
export function deriveMarketInsight(input) {
    ...
    const whatChanged = input.changePercent === null
    ...
    return {
        ...
        summary: input.forecastBias && input.forecastBias !== 'neutral'
        ...
        factors: [
            ...
            },
            {
                ...
                impact: input.forecastBias === 'bullish'
                    ? 'positive'
...
export function deriveMarketIntelligenceDigest(title, inputs, freshnessState) {
    ...
    return {
        ...
        summary: assetInsights.length > 0
        ...
        marketPulse: freshnessState === 'live'
...
function mapRecommendationAction(input) {
...
export function deriveInvestmentRecommendation(input) {
