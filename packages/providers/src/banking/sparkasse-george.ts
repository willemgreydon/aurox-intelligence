...
export type SparkasseGeorgeConnectionCapability = {
  ...
  supportedScopes: Array<'balances' | 'transactions' | 'payments' | 'multibanking'>;
...
export function getSparkasseGeorgeConnectionCapability(): SparkasseGeorgeConnectionCapability {
  ...
  if (hasPartnerCredentials) {
    return {
      ...
      disclosure:
        'Partner-style Erste open banking connectivity is configured for consent-based business account data access. Payment initiation still requires explicit user authorization and downstream execution handling.',
  ...
  return {
    ...
    disclosure:
      'Direct PSD2-style account access requires explicit user consent and usually a regulated AIS/PIS path or an approved partner setup. The app must not imply bank connectivity until onboarding is completed.',
