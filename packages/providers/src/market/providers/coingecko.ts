...
export async function fetchCoinGeckoQuote(symbol: string): Promise<MarketQuote> {
  ...
  const response = await fetchJson<CoinGeckoSimplePriceResponse>(url, { headers: getHeaders() });
...
export async function fetchCoinGeckoHistory(symbol: string): Promise<HistoricalBar[]> {
  ...
  const response = await fetchJson<CoinGeckoMarketChartResponse>(url, { headers: getHeaders() });
...
export async function fetchCoinGeckoMetadata(symbol: string): Promise<AssetMetadata> {
  const coinId = resolveCoinGeckoId(symbol);
  ...
  const result = await fetchJson<CoinGeckoCoinResponse>(url, { headers: getHeaders() });
...
export async function fetchCoinGeckoGlobalMetrics(): Promise<CryptoGlobalMetrics> {
  if (!isCoinGeckoConfigured()) {
    throw createMissingConfigError('coingecko', 'CoinGecko is not configured.');
  }

  const url = 'https://pro-api.coingecko.com/api/v3/global';
  const response = await fetchJson<CoinGeckoGlobalResponse>(url, { headers: getHeaders() });
