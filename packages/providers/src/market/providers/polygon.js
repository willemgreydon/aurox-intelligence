...
export async function fetchPolygonHistory(symbol, from, to) {
    const providerSymbol = resolvePolygonSymbol(symbol);
    if (!providerSymbol) {
        throw createMissingConfigError('polygon', `Polygon does not support symbol ${symbol}.`);
    }
    ...
    const url = buildUrl(`https://api.polygon.io/v2/aggs/ticker/${providerSymbol}/range/1/day/${start.toISOString().slice(0, 10)}/${end.toISOString().slice(0, 10)}`, {
        adjusted: 'true',
        sort: 'asc',
        limit: 5000,
        apikey: apiKey,
...
export async function fetchPolygonMetadata(symbol) {
    const providerSymbol = resolvePolygonSymbol(symbol);
    if (!providerSymbol) {
