...
function isPlainEquityLike(symbol: string) {
  return /^[A-Z.\-]{1,20}$/.test(symbol);
}

function normalizeForexPair(raw: string): string | null {
...
function normalizeCryptoFromDashedPair(pair: string): string | null {
...
function normalizeCryptoFromConcatenated(pair: string): string | null {
...
export function detectCanonicalAssetKind(symbol: string): MarketAssetKind {
...
export function normalizeCryptoPair(symbol: string) {
  const normalized = normalizeMarketSymbol(symbol);
...
export function resolvePolygonSymbol(symbol: string) {
  const normalized = normalizeMarketSymbol(symbol);
...
export function resolveTwelveDataSymbol(symbol: string) {
  const normalized = normalizeMarketSymbol(symbol);
...
export function resolveFinnhubSymbol(symbol: string) {
  const normalized = normalizeMarketSymbol(symbol);
  const assetKind = detectCanonicalAssetKind(normalized);
