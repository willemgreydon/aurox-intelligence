...
function getHealthState(provider) {
...
export function recordProviderFailure(provider, latencyMs) {
    const state = getHealthState(provider);
...
function supportsAssetKind(provider, assetKind) {
...
function supportsReadKind(provider, kind) {
    const entry = PROVIDER_REGISTRY[provider];
    return entry.supportedKinds.includes(kind);
}
function getReadPreferenceBoost(provider, kind, assetKind, normalizedSymbol) {
...
export function resolveProvidersForRead(kind, assetKind, symbol) {
