...
export function mapFinnhubObservation(symbol: string, input: FinnhubQuoteResponse): MarketQuote {
  const normalizedSymbol = normalizeMarketSymbol(symbol);
  const parsed = finnhubQuoteSchema.parse(input);

  const observation: MarketQuote = {
...
export function mapEodhdObservation(symbol: string, input: EodhdRealTimeResponse): MarketQuote {
  const normalizedSymbol = normalizeMarketSymbol(symbol);
...
export function mapFinnhubHistory(symbol: string, input: FinnhubCandleResponse): HistoricalBar[] {
  const normalizedSymbol = normalizeMarketSymbol(symbol);
...
export function mapEodhdHistory(symbol: string, input: EodhdHistoricalPointResponse[]): HistoricalBar[] {
  const normalizedSymbol = normalizeMarketSymbol(symbol);
