---
name: aurox-signal-engine
description: Design deterministic signal scoring pipelines with explainable factor contributions for market assets.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-signal-engine Skill
## Purpose
Design deterministic signal scoring pipelines with explainable factor contributions for market assets.
## When To Use
- Build or modify score computation, signal labels, confidence logic, and indicator derivations.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Pure UI style work with no signal math or data-contract changes.
...
2. Inspect domain logic in focused packages: packages/signals; packages/api-contracts; apps/web/server/services; apps/web/components/signals.
...
- Keep score range clamped to -1..1; expose confidence 0..1; include indicator contribution outputs.
...
## Safety Rules
- No AI-only score generation; deterministic math must be source of truth.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/signals; packages/api-contracts; apps/web/server/services; apps/web/components/signals
## Expected Output Format
`	ext
Skill: aurox-signal-engine
Scope:
- ...
Contracts/Types:
- created/updated ...
