---
name: aurox-position-sizing
description: Define and enforce deterministic position sizing policies before order simulation/execution.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-position-sizing Skill
## Purpose
Define and enforce deterministic position sizing policies before order simulation/execution.
## When To Use
- Implement max position, risk-budget sizing, or quantity constraints.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Pure dashboard cosmetics.
...
2. Inspect domain logic in focused packages: packages/agents/src/risk; packages/agents/src/policies; apps/web/server/services/trade-execution-service.ts.
...
- Support quantity/notional/risk-budget paths with explicit validation errors.
...
## Safety Rules
- Sizing failures must block execution, never auto-correct silently.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/risk; packages/agents/src/policies; apps/web/server/services/trade-execution-service.ts
## Expected Output Format
`	ext
Skill: aurox-position-sizing
Scope:
- ...
Contracts/Types:
- created/updated ...
