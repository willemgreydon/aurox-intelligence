---
name: aurox-data-normalization
description: Enforce canonical symbol and asset normalization at ingestion/provider boundaries.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-data-normalization Skill
## Purpose
Enforce canonical symbol and asset normalization at ingestion/provider boundaries.
## When To Use
- Modify canonicalization rules or provider symbol harmonization.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Component-only edits.
...
2. Inspect domain logic in focused packages: packages/ingestion/src/canonicalize.ts; packages/providers/src/market/provider-symbols.ts.
...
- Normalize once in provider/ingestion layer; expose canonical symbols downstream.
...
## Safety Rules
- Do not duplicate normalization logic in UI/routes/services.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/ingestion/src/canonicalize.ts; packages/providers/src/market/provider-symbols.ts
## Expected Output Format
`	ext
Skill: aurox-data-normalization
Scope:
- ...
Contracts/Types:
- created/updated ...
