---
name: aurox-agent-audit-log
description: Maintain audit log schemas and event emission for autonomous and assisted decisions.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-agent-audit-log Skill
## Purpose
Maintain audit log schemas and event emission for autonomous and assisted decisions.
## When To Use
- Implement decision audit payloads, persistence fields, and trace output.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Standalone chart work.
...
2. Inspect domain logic in focused packages: packages/agents/src/types/audit-types.ts; packages/db migrations; apps/web/server/services/ai-simulation-agent-service.ts.
...
- Include traceId, mode, symbol, risk checks, policy verdicts, timestamps.
...
## Safety Rules
- No decision path without audit emission for autonomous/live-capable flows.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/types/audit-types.ts; packages/db migrations; apps/web/server/services/ai-simulation-agent-service.ts
## Expected Output Format
`	ext
Skill: aurox-agent-audit-log
Scope:
- ...
Contracts/Types:
- created/updated ...
