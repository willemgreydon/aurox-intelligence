---
name: aurox-exposure-tracking
description: Track gross/net exposure and concentration by asset class, symbol, and strategy lane.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-exposure-tracking Skill
## Purpose
Track gross/net exposure and concentration by asset class, symbol, and strategy lane.
## When To Use
- Build exposure summaries, concentration warnings, or exposure filters.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Provider adapter auth setup changes.
...
2. Inspect domain logic in focused packages: apps/web/server/mappers/portfolio-mapper.ts; packages/db; packages/agents/src/risk.
...
- Expose exposure as deterministic percentages and notionals.
...
## Safety Rules
- Missing price data must degrade exposure confidence and display warnings.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: apps/web/server/mappers/portfolio-mapper.ts; packages/db; packages/agents/src/risk
## Expected Output Format
`	ext
Skill: aurox-exposure-tracking
Scope:
- ...
Contracts/Types:
- created/updated ...
