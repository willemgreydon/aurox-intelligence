---
name: aurox-agent-decision-engine
description: Build deterministic agent decision synthesis from signal/risk/policy inputs.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-agent-decision-engine Skill
## Purpose
Build deterministic agent decision synthesis from signal/risk/policy inputs.
## When To Use
- Change intent scoring, hold/open/close/rebalance mapping, confidence gating.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Provider key management tasks.
...
2. Inspect domain logic in focused packages: packages/agents/src/intelligence; packages/agents/src/workflows.
...
- Decision packets require intent, confidence, reasoning, and risk checks.
...
## Safety Rules
- Low confidence or failed checks forces HOLD/approval-required.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/intelligence; packages/agents/src/workflows
## Expected Output Format
`	ext
Skill: aurox-agent-decision-engine
Scope:
- ...
Contracts/Types:
- created/updated ...
