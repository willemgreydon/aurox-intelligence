---
name: aurox-order-router
description: Route orders to simulation/paper/live adapters through typed mode gates.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-order-router Skill
## Purpose
Route orders to simulation/paper/live adapters through typed mode gates.
## When To Use
- Build routing logic by asset kind, mode, and broker capability.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Signal formula changes without routing impact.
...
2. Inspect domain logic in focused packages: packages/agents/src/adapters; apps/web/server/lib/brokers/live-execution-adapter.ts; packages/agents/src/execution.
...
- Routing must be explicit and idempotent with deterministic fallback.
...
## Safety Rules
- No direct UI-to-broker calls; mode gate required for every route.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/adapters; apps/web/server/lib/brokers/live-execution-adapter.ts; packages/agents/src/execution
## Expected Output Format
`	ext
Skill: aurox-order-router
Scope:
- ...
Contracts/Types:
- created/updated ...
