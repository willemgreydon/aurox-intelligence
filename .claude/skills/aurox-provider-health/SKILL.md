---
name: aurox-provider-health
description: Maintain provider health reporting, connectivity checks, and fallback signals.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-provider-health Skill
## Purpose
Maintain provider health reporting, connectivity checks, and fallback signals.
## When To Use
- Update provider health page/service and status propagation.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Execution policy tuning with no provider dependency.
...
2. Inspect domain logic in focused packages: apps/web/server/services/broker-health-service.ts; packages/providers/src/shared; apps/web/app/admin.
...
- Report reachability, freshness, error rate, and fallback state explicitly.
...
## Safety Rules
- Health degradation must not silently masquerade as nominal.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: apps/web/server/services/broker-health-service.ts; packages/providers/src/shared; apps/web/app/admin
## Expected Output Format
`	ext
Skill: aurox-provider-health
Scope:
- ...
Contracts/Types:
- created/updated ...
