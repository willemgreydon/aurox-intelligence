---
name: aurox-risk-engine
description: Build centralized trade and portfolio risk scoring used before any order path.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-risk-engine Skill
## Purpose
Build centralized trade and portfolio risk scoring used before any order path.
## When To Use
- Implement aggregated risk score and level outputs for decisions.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Standalone chart styling tasks.
...
2. Inspect domain logic in focused packages: packages/agents/src/risk; packages/agents/src/policies; apps/web/components/invest.
...
- Return low/medium/high/extreme with explainable contributing checks.
...
## Safety Rules
- Any failed hard risk check must block trading intent.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/risk; packages/agents/src/policies; apps/web/components/invest
## Expected Output Format
`	ext
Skill: aurox-risk-engine
Scope:
- ...
Contracts/Types:
- created/updated ...
