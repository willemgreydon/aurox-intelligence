---
name: aurox-ranking-engine
description: Create cross-asset ranking models with stable score ordering and confidence handling.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-ranking-engine Skill
## Purpose
Create cross-asset ranking models with stable score ordering and confidence handling.
## When To Use
- Rank stocks/ETFs/crypto across shared scoring dimensions.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Single-asset detail UI changes with no ranking impact.
...
2. Inspect domain logic in focused packages: packages/ai-market-intelligence; packages/api-contracts/src/ranking; apps/web/app/markets.
...
- Use deterministic sort tie-breakers; include rank, score, confidence, recommendation fields.
...
## Safety Rules
- Handle missing data with degraded confidence and explicit caveats.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/ai-market-intelligence; packages/api-contracts/src/ranking; apps/web/app/markets
## Expected Output Format
`	ext
Skill: aurox-ranking-engine
Scope:
- ...
Contracts/Types:
- created/updated ...
