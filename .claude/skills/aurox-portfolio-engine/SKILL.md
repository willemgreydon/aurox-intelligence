---
name: aurox-portfolio-engine
description: Implement portfolio read/write behavior for simulation-safe holdings, valuation, and allocation views.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-portfolio-engine Skill
## Purpose
Implement portfolio read/write behavior for simulation-safe holdings, valuation, and allocation views.
## When To Use
- Change position views, portfolio summaries, or allocation computation.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Provider-only ingestion changes without portfolio impact.
...
2. Inspect domain logic in focused packages: packages/db/src/repositories; apps/web/server/queries/portfolio-query.ts; apps/web/server/mappers/portfolio-mapper.ts; apps/web/app/portfolio.
...
- Preserve deterministic balances and allocation math; expose stable view models through contracts.
...
## Safety Rules
- No silent mutation of holdings; every state update must remain auditable.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/db/src/repositories; apps/web/server/queries/portfolio-query.ts; apps/web/server/mappers/portfolio-mapper.ts; apps/web/app/portfolio
## Expected Output Format
`	ext
Skill: aurox-portfolio-engine
Scope:
- ...
Contracts/Types:
- created/updated ...
