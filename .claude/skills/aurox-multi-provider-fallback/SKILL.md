---
name: aurox-multi-provider-fallback
description: Implement deterministic provider fallback and merge policies.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-multi-provider-fallback Skill
## Purpose
Implement deterministic provider fallback and merge policies.
## When To Use
- Build fallback order, timeout policies, and stale-data degradation.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Standalone copy updates.
...
2. Inspect domain logic in focused packages: packages/providers/src/market/routing.ts; packages/providers/src/shared.
...
- Define provider priority, timeout, retry, and stale handling explicitly.
...
## Safety Rules
- Fallback must preserve provenance and freshness labels.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/providers/src/market/routing.ts; packages/providers/src/shared
## Expected Output Format
`	ext
Skill: aurox-multi-provider-fallback
Scope:
- ...
Contracts/Types:
- created/updated ...
