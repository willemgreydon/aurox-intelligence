---
name: aurox-agent-core
description: Define core agent contracts, context handling, and deterministic orchestration boundaries.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-agent-core Skill
## Purpose
Define core agent contracts, context handling, and deterministic orchestration boundaries.
## When To Use
- Edit agent context, result types, and core orchestration structures.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Pure route-level UI tasks.
...
2. Inspect domain logic in focused packages: packages/agents/src/types/agent-types.ts; packages/agents/src/core.
...
- Keep agent IO typed and explicit; avoid hidden shared mutable state.
...
## Safety Rules
- All agent outputs must remain auditable and policy-checkable.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/types/agent-types.ts; packages/agents/src/core
## Expected Output Format
`	ext
Skill: aurox-agent-core
Scope:
- ...
Contracts/Types:
- created/updated ...
