---
name: aurox-agent-supervisor
description: Implement supervisory controls that gate and coordinate agent decisions safely.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-agent-supervisor Skill
## Purpose
Implement supervisory controls that gate and coordinate agent decisions safely.
## When To Use
- Update broker-supervisor and mode-policy integration.
...
2. Inspect domain logic in focused packages: packages/agents/src/broker/broker-supervisor-agent.ts; packages/agents/src/policies.
...
- Supervisor must evaluate policy, risk, and readiness before execution dispatch.
...
## Safety Rules
- Supervisor denies by default on incomplete context.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/broker/broker-supervisor-agent.ts; packages/agents/src/policies
## Expected Output Format
`	ext
Skill: aurox-agent-supervisor
Scope:
- ...
Contracts/Types:
- created/updated ...
