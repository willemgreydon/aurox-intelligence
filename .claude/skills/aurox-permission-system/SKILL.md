---
name: aurox-permission-system
description: Enforce user/operator permission boundaries for trading capabilities and mode changes.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-permission-system Skill
## Purpose
Enforce user/operator permission boundaries for trading capabilities and mode changes.
## When To Use
- Add permission checks around execution mode, autonomous toggles, and high-risk actions.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Provider ingestion-only changes.
...
2. Inspect domain logic in focused packages: apps/web/server/auth; apps/web/server/actions; packages/db schema for user preferences.
...
- Permissions must be explicit, typed, and denied by default.
...
## Safety Rules
- Never infer elevated permissions from client-side state.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: apps/web/server/auth; apps/web/server/actions; packages/db schema for user preferences
## Expected Output Format
`	ext
Skill: aurox-permission-system
Scope:
- ...
Contracts/Types:
- created/updated ...
