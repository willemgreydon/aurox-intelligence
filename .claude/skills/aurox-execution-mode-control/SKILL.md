---
name: aurox-execution-mode-control
description: Manage execution mode state across simulation, paper, and gated live variants.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-execution-mode-control Skill
## Purpose
Manage execution mode state across simulation, paper, and gated live variants.
## When To Use
- Implement execution-mode registry, switch UI, and mode propagation to order routes.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Standalone docs updates without code changes.
...
2. Inspect domain logic in focused packages: packages/agents/src/execution; apps/web/components/broker/execution-mode-switch.tsx; apps/web/server/config.
...
- Default mode is simulation; all order actions resolve through mode gate.
...
## Safety Rules
- live-autonomous remains disabled until explicit opt-in and readiness pass.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/execution; apps/web/components/broker/execution-mode-switch.tsx; apps/web/server/config
## Expected Output Format
`	ext
Skill: aurox-execution-mode-control
Scope:
- ...
Contracts/Types:
- created/updated ...
