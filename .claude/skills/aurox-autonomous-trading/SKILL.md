---
name: aurox-autonomous-trading
description: Design autonomous agent architecture in dry-run/simulation mode with hard safety gates.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-autonomous-trading Skill
## Purpose
Design autonomous agent architecture in dry-run/simulation mode with hard safety gates.
## When To Use
- Add autonomous capabilities, dry-run workflows, mode disable defaults.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Manual-only order form text changes.
...
2. Inspect domain logic in focused packages: packages/agents/src/autonomous; packages/agents/src/execution; apps/web/components/broker.
...
- Autonomous outputs must include requiresApproval and executionMode fields.
...
## Safety Rules
- Live autonomous remains disabled unless every readiness gate passes.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/autonomous; packages/agents/src/execution; apps/web/components/broker
## Expected Output Format
`	ext
Skill: aurox-autonomous-trading
Scope:
- ...
Contracts/Types:
- created/updated ...
