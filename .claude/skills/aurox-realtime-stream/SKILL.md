---
name: aurox-realtime-stream
description: Prepare streaming market updates with graceful fallback to snapshot polling.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-realtime-stream Skill
## Purpose
Prepare streaming market updates with graceful fallback to snapshot polling.
## When To Use
- Add realtime feed orchestration or stream-to-read-model updates.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Risk-policy-only changes.
...
2. Inspect domain logic in focused packages: packages/providers; apps/web/server/services; apps/web/components/markets.
...
- Streams must map into the same contracts as snapshots.
...
## Safety Rules
- On stream disconnect, degrade to cached/polling mode with visible state.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/providers; apps/web/server/services; apps/web/components/markets
## Expected Output Format
`	ext
Skill: aurox-realtime-stream
Scope:
- ...
Contracts/Types:
- created/updated ...
