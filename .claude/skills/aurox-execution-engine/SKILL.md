---
name: aurox-execution-engine
description: Implement order execution orchestration with policy, risk, and adapter layers.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-execution-engine Skill
## Purpose
Implement order execution orchestration with policy, risk, and adapter layers.
## When To Use
- Modify unified/simulation workflow execution behavior.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Unrelated docs-only tasks unless paired with execution changes.
...
2. Inspect domain logic in focused packages: packages/agents/src/workflows; apps/web/server/services/trade-execution-service.ts.
...
- Sequence: validate -> policy -> risk -> adapter -> reconcile -> audit.
...
## Safety Rules
- Fail closed on adapter or reconciliation uncertainty.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/workflows; apps/web/server/services/trade-execution-service.ts
## Expected Output Format
`	ext
Skill: aurox-execution-engine
Scope:
- ...
Contracts/Types:
- created/updated ...
