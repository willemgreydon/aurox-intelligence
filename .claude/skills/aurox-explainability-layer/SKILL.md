---
name: aurox-explainability-layer
description: Maintain explainability outputs for signal, recommendation, and risk decisions.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-explainability-layer Skill
## Purpose
Maintain explainability outputs for signal, recommendation, and risk decisions.
## When To Use
- Attach rationale bullets, factor attribution, and warnings to decision models.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Low-level auth cookie changes.
...
2. Inspect domain logic in focused packages: packages/forecasting/src/explainability; packages/agents/src/types/audit-types.ts; apps/web/components/invest/recommendation-card.tsx.
...
- Every recommendation must include reasons, risk warnings, and mode provenance.
...
## Safety Rules
- No black-box recommendation without deterministic trace metadata.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/forecasting/src/explainability; packages/agents/src/types/audit-types.ts; apps/web/components/invest/recommendation-card.tsx
## Expected Output Format
`	ext
Skill: aurox-explainability-layer
Scope:
- ...
Contracts/Types:
- created/updated ...
