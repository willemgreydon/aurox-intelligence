---
name: aurox-factor-engine
description: Implement and maintain factor-model calculations used by ranking and recommendation layers.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-factor-engine Skill
## Purpose
Implement and maintain factor-model calculations used by ranking and recommendation layers.
## When To Use
- Adding momentum/value/quality/regime factors or reweighting factor composites.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Broker execution plumbing unrelated to factor construction.
...
2. Inspect domain logic in focused packages: packages/signals; packages/forecasting; packages/agents; packages/api-contracts.
...
- Define factor inputs explicitly; normalize factors before aggregation; document each factor impact.
...
## Safety Rules
- Reject hidden factor side-effects and non-auditable transformations.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/signals; packages/forecasting; packages/agents; packages/api-contracts
## Expected Output Format
`	ext
Skill: aurox-factor-engine
Scope:
- ...
Contracts/Types:
- created/updated ...
