---
name: aurox-drawdown-control
description: Enforce drawdown guardrails and circuit-breaker logic for simulation and future live paths.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-drawdown-control Skill
## Purpose
Enforce drawdown guardrails and circuit-breaker logic for simulation and future live paths.
## When To Use
- Change max drawdown policies or drawdown-triggered holds.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Provider quote mapper tweaks with no risk policy effect.
...
2. Inspect domain logic in focused packages: packages/agents/src/risk/drawdown-guard-agent.ts; packages/agents/src/policies.
...
- Evaluate drawdown against configured thresholds and emit explicit reasons.
...
## Safety Rules
- Default fail-safe is HOLD/no execution under drawdown uncertainty.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/risk/drawdown-guard-agent.ts; packages/agents/src/policies
## Expected Output Format
`	ext
Skill: aurox-drawdown-control
Scope:
- ...
Contracts/Types:
- created/updated ...
