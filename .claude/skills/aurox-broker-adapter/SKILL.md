---
name: aurox-broker-adapter
description: Create and maintain broker adapter abstractions and safe provider stubs.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-broker-adapter Skill
## Purpose
Create and maintain broker adapter abstractions and safe provider stubs.
## When To Use
- Add Binance/Coinbase/stock broker capability discovery and order stubs.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Portfolio UI rendering changes.
...
2. Inspect domain logic in focused packages: packages/agents/src/broker; apps/web/server/lib/brokers.
...
- Expose supportsCrypto/equities/ETFs/order types/paper/live capabilities.
...
## Safety Rules
- Never hardcode secrets; live adapters disabled by default.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/broker; apps/web/server/lib/brokers
## Expected Output Format
`	ext
Skill: aurox-broker-adapter
Scope:
- ...
Contracts/Types:
- created/updated ...
