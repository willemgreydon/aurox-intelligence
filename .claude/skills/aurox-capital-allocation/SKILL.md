---
name: aurox-capital-allocation
description: Maintain lane-aware capital allocation and exposure budgets across simulation workflows.
allowed-tools: Read, Grep, Glob, LS, Edit, MultiEdit, Write, Bash
---
# aurox-capital-allocation Skill
## Purpose
Maintain lane-aware capital allocation and exposure budgets across simulation workflows.
## When To Use
- Adjust per-lane budgets or portfolio cash allocation controls.
- Changes span query -> mapper -> service -> route -> UI and require architecture-safe delivery.
## When Not To Use
- Non-financial layout updates.
...
2. Inspect domain logic in focused packages: packages/agents/src/types/broker-types.ts; apps/web/server/services/simulation-service.ts; apps/web/server/services/simulation-activity-lanes.ts.
...
- Allocation limits must be explicit and testable per lane.
...
## Safety Rules
- Never allow allocation overrun without a hard block and reason.
...
- pps/web/app
- pps/web/components
- Skill-specific focus: packages/agents/src/types/broker-types.ts; apps/web/server/services/simulation-service.ts; apps/web/server/services/simulation-activity-lanes.ts
## Expected Output Format
`	ext
Skill: aurox-capital-allocation
Scope:
- ...
Contracts/Types:
- created/updated ...
